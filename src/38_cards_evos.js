...
const EVOLUTIONS = [
 ...
 {evo:true, id:'afterburner',   icon:'🚀', text:'Afterburner',      desc:'Kill haste lasts 3s and runs hotter',       need:['speed','adrenaline']},
...
const _STAT_META = {
    ...
    adrenaline: { icon:'💨', label:'Adrenaline Rush', desc:'+1.5s kill-haste per stack' },
...
function syncPlayerTankParts(){
    ...
    for (let i = 0; i < extraBarrels; i++) {
        const side = (i % 2 === 0 ? 1 : -1) * (0.38 + Math.floor(i / 2) * 0.28);
        kitCyl(tur, 0.09, 0.11, 2.6, side, 0.5, 2.2, dark, Math.PI / 2, 0, 0);
    }
    if ((ps.missile || 0) > 0) {
